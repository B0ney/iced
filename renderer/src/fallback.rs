...
mod geometry {
        ...
        B: geometry::Renderer,
    {
        ...
        fn supports_cache(
            &self,
            cache: &Geometry<<A::Geometry as Cached>::Cache, <B::Geometry as Cached>::Cache>,
        ) -> bool {
            matches!(
                (self, cache),
        ...
        B: Cached,
    {
        ...
        fn cache(self, group: cache::Group, previous: Option<Self::Cache>) -> Self::Cache {
            match (self, previous) {
                ...
                (Self::Primary(geometry), _) => Geometry::Primary(geometry.cache(group, None)),
                (Self::Secondary(geometry), Some(Geometry::Secondary(previous))) => {
                    Geometry::Secondary(geometry.cache(group, Some(previous)))
                }
                (Self::Secondary(geometry), _) => Geometry::Secondary(geometry.cache(group, None)),
            }
        }
    }
